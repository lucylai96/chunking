...
chunkers = [ 2     3     6     9    14    17    19    22    30    36    37    41    44    49    50    62    66 73 76];
nonchunkers=[ 1     5     7     9    10    17    22    23    29    34    37    38    43    48];
%data = data(chunkers);
%exportgraphics(gcf,[pwd '/figures/raw/fig1.pdf'], 'ContentType', 'vector');
simdata = [];
switch fig
    case 'supp' % task
        chunkers4 = [2, 3, 4, 6, 9, 12, 14, 15, 16, 17, ...
            19, 21, 22, 25, 27, 29, 30, 32, 35, 37, ...
            41, 44, 47, 49, 50, 51, 53, 57, 58, 59, ...
            62, 63, 64, 65, 66, 73, 75, 76];
        chunkers6 = [2, 3, 6, 9, 10, 14, 17, 18, 19, 20, ...
            22, 24, 29, 30, 36, 37, 39, 41, 44, 45, ...
            46, 49, 50, 56, 60, 61, 62, 66, 67, 70, ...
            71, 72, 73, 74, 76];
        %chunkers = [2, 3, 6, 9, 14, 17, 19, 22, 29, 30, 37, 41, 44, 49, 50, 62, 66, 73, 76];
        chunkers = [7    10    11    14    23    26    31    33    34    37    38    42    43    47    54    55    56    58    64    67    68];
        % nonchunkers =[1,5, 7, 8,13,23,28,31,33,  34, 40,  42,  48, 52, 54, 55, 68, 69];
        %data = data(chunkers);

        analysis_data('supp1', data)

        load actionChunk_data.mat;
        data = data(nonchunkers);
        analysis_data('supp12', data)
        %data = data(setdiff(1:76,chunkers4));
        %data = data(setdiff(1:76,chunkers6));        analysis_data('supp1', data)
